...
`.github/workflows/build.yml`: Linux даёт `.tar.gz`, macOS — `.dmg`,
Windows — установщик. Не по каждому пушу: час с лишним на платформу, так что
запуск руками из вкладки Actions (можно попросить одну платформу) или по тегу.
Каждая сборка перед упаковкой собирает `examples/hello`, гоняет его на машине
и проверяет, что он напечатал своё.
